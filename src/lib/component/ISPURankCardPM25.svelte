<script>
	...
	export let ispuTopRank;
	export let svgloading;
</script>

<div class="fixed flex mt-16 flex-end">
	<div
		class="w-80 ml-7 mt-7 rounded-lg border border-gt-color-dark-3 border-opacity-80 shadow-sm shadow-black justify-end overflow-hidden"
	>
		<!-- Card Header -->
		<div
		...
		<div
			...
			class="flex flex-col gap-2 p-2 bg-gt-color-dark bg-opacity-80 backdrop-blur-[2px]"
		>
			{#if $isLoading}
				<div in:fade={{ duration: 100 }} class="justify-center items-center flex">
					{@html svgloading}
				</div>
			{:else if ispuTopRank.length > 0}
				{#each ispuTopRank as ispuRank, index}
					{#if index < 4}
						<div
							in:fade={{ duration: 200 }}
							class="flex flex-row gap-5 py-1.5 px-2 items-center justify-between"
						>
							<div class="flex grow shrink basis-0 h-5 justify-center items-center gap-2">
								<!-- Parameter -->
								<div class="w-fit font-medium text-xs text-gt-color-gray uppercase">
									{ispuRank.parameter}
								</div>
								<!-- End of Parameter -->
								...
								<!-- Station Column -->
								<div
									class="grow shrink basis-0 flex-col justify-end items-start gap-1.5 inline-flex"
								>
									<div class="inline-flex self-stretch justify-between items-center gap-3">
										...
										<div class="flex flex-row gap-3 items-center">
											...
											<span class="font-medium text-xs text-gt-color-gray items-center">
												{ispuRank.name}
											</span>
										</div>
										<!-- End of Station Name -->
							...
							{#if ispuRank.category === 'baik'}
								<div class="flex bg-[#22AD5C] px-2 py-0.5 justify-center items-center rounded-full">
									<span class="font-medium text-sm text-center text-gt-color-gray">
										{ispuRank.value}
									</span>
								</div>
						...
						{#if ispuTopRank.length !== index + 1}
							<hr class="border-gt-color-dark-3 bg-opacity-80" />
						{/if}
					{/if}
				{/each}
			{:else}
				<div in:fade={{ duration: 200 }} class="justify-center items-center flex">
					<p class="text-xs text-gt-color-gray">Tidak ada data untuk ditampilkan</p>
				</div>
			{/if}
		</div>
		<!-- End of Card Body -->
