...
<div class="fixed flex flex-end mt-16 z-10">
	<div
		class="justify-end w-72 ml-7 mt-7 shadow-sm shadow-black border border-gt-color-dark-3-transparent rounded-lg"
	>
		<!-- Modal Body -->
		<div
			class="flex flex-col gap-3 p-3 bg-gt-color-dark bg-opacity-80 backdrop-blur-[6px] {demographyModal
				? 'max-h-[34.5rem]'
				: 'max-h-[25rem]'} rounded-lg overflow-y-auto relative scrollbar ease-in-out duration-150"
		>
			<!-- Station Identity -->

			<!-- Alamat -->
			<div class="flex flex-col gap-1 justify-between">
				<span class="font-medium text-xs text-nowrap text-gt-color-gray">Alamat:</span>
				<span class="font-normal text-xs text-gt-color-gray text-wrap"
					>{selectedAirStation?.stationData?.alamat}</span
				>
			</div>
			<!-- End of Alamat -->
			...
			<div class="flex flex-row justify-between">
				<span class="font-medium text-xs text-nowrap text-gt-color-gray">Koordinat:</span>
				<span class="font-normal text-right text-xs text-gt-color-gray text-wrap"
					>{selectedAirStation?.stationData?.lat}, {selectedAirStation?.stationData?.lon}</span
				>
			</div>
			<!-- End of Koordinat -->
			...
			<!-- Last Update -->
			<div class="flex flex-row justify-between">
				<span class="font-medium text-xs text-nowrap italic text-gt-color-gray"
					>Update Terakhir</span
				>
				<span class="ml-1 font-normal text-right text-xs text-gt-color-gray text-wrap"
					>{selectedAirStation?.stationData?.last_update
						? formatDateDmyHis(selectedAirStation?.stationData?.last_update)
						: 'Not Updated'}</span
				>
			</div>
			<!-- End of Last Update -->
				...
				<div class="flex flex-col gap-2">
					<div class="flex flex-row gap-2 items-center">
						{#if selectedAirStation.aqi.category === 'baik'}
							{#if selectedAirStation.highest.value === 0}
								<span class="h-5 w-5 bg-gray-400 rounded-full"></span>
							{:else}
						...
						<span class="font-normal text-sm text-gt-color-gray capitalize">
							{#if selectedAirStation.highest.value === 0}
								Offline
							{:else}
								{selectedAirStation.aqi.category.replace(/_/g, ' ')}
							{/if}
						</span>
					</div>
								...
								<div class="flex flex-col gap-2 rounded">
									<div class="flex flex-row w-full gap-2 items-center justify-between">
										...
										<label class="relative inline-flex items-center cursor-pointer">
											<input
												type="checkbox"
												name="population-density-toggle"
												id="population-density-toggle"
												class="sr-only peer"
