...
<div transition:fade={{ duration: 150 }} class="fixed flex mt-16 flex-end">
	<div
			...
			{#each ispuTopRank as ispuRank, index}
				{#if index < 4}
					...
					>
									...
									>
										<span class="font-medium text-center text-[#F9FAFB] text-xs">
											{#if ispuRank.tipe_stasiun === 'lowcost'}
												LC
											{:else if ispuRank.tipe_stasiun === 'manual'}
												M
						...
						{:else}
							<div class="flex bg-[#0C0C0D] px-2 py-0.5 justify-center items-center rounded-full">
								<span class="font-medium text-sm text-center text-[#F9FAFB]">{ispuRank.value}</span>
							</div>
						{/if}
						<!-- End of Badge -->
					</div>
