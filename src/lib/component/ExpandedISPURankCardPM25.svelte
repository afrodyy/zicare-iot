<script>
	...
	function actSelectJenisStasiun(value) {
		if (value === 'all') {
			...
			filteredStations = ispuRankAll.filter(
...
<div class="fixed flex flex-end mt-16 z-10">
	<div
		...
		<div class="columns-2 flex h-[24rem]">
			...
			>
				...
				>
					...
					<div class="relative inline-block text-left mt-1 w-full">
						<button
							...
							type="button"
							class="whitespace-nowrap inline-flex w-full justify-between gap-x-1.5 rounded-md bg-gt-color-dark px-3 py-2 text-sm text-[#F9FAFB] shadow-sm ring-1 ring-inset ring-gt-color-dark-3-transparent uppercase"
							id="menu-button"
							aria-expanded="true"
							aria-haspopup="true"
						>
				...
				>
					...
					<div class="relative inline-block text-left mt-1 w-full">
						<button
							on:click={() => (selectOpenJenisStasiun = !selectOpenJenisStasiun)}
							type="button"
							class="flex flex-row items-center w-full gap-x-1.5 px-3 py-2 rounded-md bg-gt-color-dark text-sm text-gt-color-gray shadow-sm justify-between border border-gt-color-dark-3-transparent text-nowrap"
						>
							{getArrayBy(stationTypes, 'value', selectedStationType).label}

							<svg
								class="size-5 ml-12 {selectOpenJenisStasiun
									? 'rotate-180'
						...
						{#if selectOpenJenisStasiun}
							<div
								transition:fly={{ y: 20, duration: 150 }}
								class="absolute max-h-36 left-0 right-0 z-10 origin-top-right rounded-md bg-gt-color-dark shadow-lg ring-1 ring-gt-color-dark-3-transparent ring-opacity-5 focus:outline-none overflow-y-auto"
								role="menu"
								aria-orientation="vertical"
								aria-labelledby="menu-button"
								tabindex="-1"
			...
			>
				<div class="self-stretch justify-end items-center gap-3 inline-flex rounded-md mb-3">
					...
					>
						<div class="text-center text-gt-color-gray text-sm font-normal">
							{!$ispuRankAsc ? 'Terburuk' : 'Terbaik'}
						</div>

						<div class="w-5 h-5 relative text-gt-color-gray">
							<svg
								xmlns="http://www.w3.org/2000/svg"
								fill="none"
								viewBox="0 0 24 24"
				...
				<div class="relative h-[300px] overflow-y-scroll scrollbar">
					<table class="w-full text-sm text-left rtl:text-right = dark:text-gray-400">
						<thead
							class="text-gt-color-gray rounded bg-gt-color-dark font-medium leading-relaxed sticky top-0 z-10"
						>
							<tr>
								...
								{#if filteredStations.length > 0}
									<tr
										...
										class={filteredStations.length - 1 > index
											? 'border-b border-gt-color-dark-3-transparent text-gt-color-gray text-md font-medium leading-relaxed'
											: ''}
									>
										...
										<td class="text-left font-medium text-md text-gt-color-gray uppercase">
											{getArrayBy(selectValueParamIspus, 'value', selectedValueParamIspu).label}
										</td>
										<td class="text-left font-medium text-md text-gt-color-gray">
											{ispuRank.name}
										</td>
										<td class="text-left">
											{#if ispuRank.category === 'baik'}
												<div class="w-fit px-2.5 py-0.5 bg-[#22AD5C] rounded-full">
													<span class="font-normal text-sm text-gt-color-gray"
														>{ispuRank[selectedValueParamIspu] || 0}
													</span>
												</div>
								...
								{:else}
									<tr in:fade={{ duration: 200 }}>
										<td
											...
											colspan="4">Data tidak ditemukan</td
										>
									</tr>
								{/if}
							{/each}
